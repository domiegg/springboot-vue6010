					...
					<el-table-column :resizable='true' :sortable='true' prop="luxiantupian" width="200" label="路线图片">
						<template slot-scope="scope">
							<div v-if="scope.row.luxiantupian">
								<img v-if="scope.row.luxiantupian.substring(0,4)=='http'" :src="scope.row.luxiantupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.luxiantupian.split(',')[0])">
								<img v-else :src="$base.url+scope.row.luxiantupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView($base.url+scope.row.luxiantupian.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
