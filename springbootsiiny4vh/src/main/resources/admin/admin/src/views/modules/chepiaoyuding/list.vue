					...
					<el-table-column :resizable='true' :sortable='true' prop="cheliangzhaopian" width="200" label="车辆照片">
						<template slot-scope="scope">
							<div v-if="scope.row.cheliangzhaopian">
								<img v-if="scope.row.cheliangzhaopian.substring(0,4)=='http'" :src="scope.row.cheliangzhaopian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.cheliangzhaopian.split(',')[0])">
								<img v-else :src="$base.url+scope.row.cheliangzhaopian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView($base.url+scope.row.cheliangzhaopian.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
