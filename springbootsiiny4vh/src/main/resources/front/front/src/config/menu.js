const menu = {
    list() {
        return [{
            "backMenu": [{
                "child": [{
                    "allButtons": ["新增", "查看", "修改", "删除"],
                    "appFrontIcon": "cuIcon-wenzi",
                    "buttons": ["新增", "查看", "修改", "删除"],
                    "menu": "用户",
                    "menuJump": "列表",
                    "tableName": "yonghu"
                }],
                "menu": "用户管理"
            }, {
                "child": [{
                    "allButtons": ["新增", "查看", "修改", "删除", "支付", "购票统计", "首页总数", "首页统计"],
                    "appFrontIcon": "cuIcon-keyboard",
                    "buttons": ["查看", "修改", "删除", "首页总数", "首页统计"],
                    "menu": "门票预定",
                    "menuJump": "列表",
                    "tableName": "menpiaoyuding"
                }],
                "menu": "门票预定管理"
            }, {
                "child": [{
                    "allButtons": ["新增", "查看", "修改", "删除"],
                    "appFrontIcon": "cuIcon-similar",
                    "buttons": ["新增", "查看", "修改", "删除"],
                    "menu": "酒店类型",
                    "menuJump": "列表",
                    "tableName": "jiudianleixing"
                }],
                "menu": "酒店类型管理"
            }, {
                "child": [{
                    "allButtons": ["新增", "查看", "修改", "删除", "查看评论", "酒店预定"],
                    "appFrontIcon": "cuIcon-phone",
                    "buttons": ["新增", "查看", "修改", "删除", "查看评论"],
                    "menu": "酒店信息",
                    "menuJump": "列表",
                    "tableName": "jiudianxinxi"
                }],
                "menu": "酒店信息管理"
            }, {
                "child": [{
                    "allButtons": ["新增", "查看", "修改", "删除", "支付", "类型占比", "首页总数", "首页统计"],
                    "appFrontIcon": "cuIcon-send",
                    "buttons": ["查看", "修改", "删除", "首页总数", "首页统计"],
                    "menu": "酒店预定",
                    "menuJump": "列表",
                    "tableName": "jiudianyuding"
                }],
                "menu": "酒店预定管理"
            }, {
                "child": [{
                    "allButtons": ["新增", "查看", "修改", "删除"],
                    "appFrontIcon": "cuIcon-cardboard",
                    "buttons": ["新增", "查看", "修改", "删除"],
                    "menu": "美食分类",
                    "menuJump": "列表",
                    "tableName": "meishifenlei"
                }],
                "menu": "美食分类管理"
            }, {
                "child": [{
                    "allButtons": ["新增", "查看", "修改", "删除", "查看评论", "美食预定"],
                    "appFrontIcon": "cuIcon-news",
                    "buttons": ["新增", "查看", "修改", "删除", "查看评论", "美食预定"],
                    "menu": "美食信息",
                    "menuJump": "列表",
                    "tableName": "meishixinxi"
                }],
                "menu": "美食信息管理"
            }, {
                "child": [{
                    "allButtons": ["新增", "查看", "修改", "删除", "支付", "美食分类", "首页总数", "首页统计"],
                    "appFrontIcon": "cuIcon-explore",
                    "buttons": ["查看", "修改", "删除", "首页总数", "首页统计"],
                    "menu": "美食预定",
                    "menuJump": "列表",
                    "tableName": "meishiyuding"
                }],
                "menu": "美食预定管理"
            }, {
                "child": [{
                    "allButtons": ["新增", "查看", "修改", "删除"],
                    "appFrontIcon": "cuIcon-present",
                    "buttons": ["新增", "查看", "修改", "删除"],
                    "menu": "论坛管理",
                    "tableName": "forum"
                }],
                "menu": "论坛管理"
            }, {
                "child": [{
                    "allButtons": ["查看", "修改"],
                    "appFrontIcon": "cuIcon-list",
                    "buttons": ["查看", "修改"],
                    "menu": "系统简介",
                    "tableName": "systemintro"
                }, {
                    "allButtons": ["新增", "查看", "修改", "删除"],
                    "appFrontIcon": "cuIcon-clothes",
                    "buttons": ["新增", "查看", "修改", "删除"],
                    "menu": "轮播图管理",
                    "tableName": "config"
                }, {
                    "allButtons": ["新增", "查看", "修改", "删除"],
                    "appFrontIcon": "cuIcon-news",
                    "buttons": ["新增", "查看", "修改", "删除"],
                    "menu": "公告信息",
                    "tableName": "news"
                }, {
                    "allButtons": ["新增", "查看", "修改", "删除"],
                    "appFrontIcon": "cuIcon-news",
                    "buttons": ["新增", "查看", "修改", "删除"],
                    "menu": "公告信息分类",
                    "tableName": "newstype"
                }, {
                    "allButtons": ["查看", "修改"],
                    "appFrontIcon": "cuIcon-paint",
                    "buttons": ["查看", "修改"],
                    "menu": "关于我们",
                    "tableName": "aboutus"
                }],
                "menu": "系统管理"
            }],
            "frontMenu": [{
                "child": [{
                    "allButtons": ["新增", "查看", "修改", "删除", "预定"],
                    "appFrontIcon": "cuIcon-brand",
                    "buttons": ["查看", "预定"],
                    "menu": "旅游路线列表",
                    "menuJump": "列表",
                    "tableName": "lvyouluxian"
                }],
                "menu": "旅游路线模块"
            }, {
                "child": [{
                    "allButtons": ["新增", "查看", "修改", "删除", "查看评论", "车票预定"],
                    "appFrontIcon": "cuIcon-pic",
                    "buttons": ["查看", "车票预定", "查看评论"],
                    "menu": "车票信息列表",
                    "menuJump": "列表",
                    "tableName": "chepiaoxinxi"
                }],
                "menu": "车票信息模块"
            }, {
                "child": [{
                    "allButtons": ["新增", "查看", "修改", "删除", "查看评论", "酒店预定"],
                    "appFrontIcon": "cuIcon-wenzi",
                    "buttons": ["查看", "酒店预定", "查看评论"],
                    "menu": "酒店信息列表",
                    "menuJump": "列表",
                    "tableName": "jiudianxinxi"
                }],
                "menu": "酒店信息模块"
            }, {
                "child": [{
                    "allButtons": ["新增", "查看", "修改", "删除", "查看评论", "美食预定"],
                    "appFrontIcon": "cuIcon-full",
                    "buttons": ["查看", "美食预定", "查看评论"],
                    "menu": "美食信息列表",
                    "menuJump": "列表",
                    "tableName": "meishixinxi"
                }],
                "menu": "美食信息模块"
            }, {
                "child": [{
                    "allButtons": ["新增", "查看", "修改", "删除"],
                    "appFrontIcon": "cuIcon-goods",
                    "buttons": ["查看"],
                    "menu": "旅游攻略列表",
                    "menuJump": "列表",
                    "tableName": "lvyougonglve"
                }],
                "menu": "旅游攻略模块"
            }],
            "hasBackLogin": "是",
            "hasBackRegister": "否",
            "hasFrontLogin": "否",
            "hasFrontRegister": "否",
            "roleName": "管理员",
            "tableName": "users"
        }, {
            "backMenu": [{
                "child": [{
                    "allButtons": ["新增", "查看", "修改", "删除", "支付", "类型占比", "首页总数", "首页统计"],
                    "appFrontIcon": "cuIcon-send",
                    "buttons": ["查看", "支付"],
                    "menu": "酒店预定",
                    "menuJump": "列表",
                    "tableName": "jiudianyuding"
                }],
                "menu": "酒店预定管理"
            }, {
                "child": [{
                    "allButtons": ["新增", "查看", "修改", "删除", "支付", "美食分类", "首页总数", "首页统计"],
                    "appFrontIcon": "cuIcon-explore",
                    "buttons": ["查看", "支付"],
                    "menu": "美食预定",
                    "menuJump": "列表",
                    "tableName": "meishiyuding"
                }],
                "menu": "美食预定管理"
            }],
            "frontMenu": [{
                "child": [{
                    "allButtons": ["新增", "查看", "修改", "删除", "预定"],
                    "appFrontIcon": "cuIcon-brand",
                    "buttons": ["查看", "预定"],
                    "menu": "旅游路线列表",
                    "menuJump": "列表",
                    "tableName": "lvyouluxian"
                }],
                "menu": "旅游路线模块"
            }, {
                "child": [{
                    "allButtons": ["新增", "查看", "修改", "删除", "查看评论", "车票预定"],
                    "appFrontIcon": "cuIcon-pic",
                    "buttons": ["查看", "车票预定", "查看评论"],
                    "menu": "车票信息列表",
                    "menuJump": "列表",
                    "tableName": "chepiaoxinxi"
                }],
                "menu": "车票信息模块"
            }, {
                "child": [{
                    "allButtons": ["新增", "查看", "修改", "删除", "查看评论", "预定"],
                    "appFrontIcon": "cuIcon-qrcode",
                    "buttons": ["查看", "预定", "查看评论"],
                    "menu": "景点信息列表",
                    "menuJump": "列表",
                    "tableName": "jingdianxinxi"
                }],
                "menu": "景点信息模块"
            }, {
                "child": [{
                    "allButtons": ["新增", "查看", "修改", "删除", "查看评论", "酒店预定"],
                    "appFrontIcon": "cuIcon-wenzi",
                    "buttons": ["查看", "酒店预定", "查看评论"],
                    "menu": "酒店信息列表",
                    "menuJump": "列表",
                    "tableName": "jiudianxinxi"
                }],
                "menu": "酒店信息模块"
            }, {
                "child": [{
                    "allButtons": ["新增", "查看", "修改", "删除", "查看评论", "美食预定"],
                    "appFrontIcon": "cuIcon-full",
                    "buttons": ["查看", "美食预定", "查看评论"],
                    "menu": "美食信息列表",
                    "menuJump": "列表",
                    "tableName": "meishixinxi"
                }],
                "menu": "美食信息模块"
            }, {
                "child": [{
                    "allButtons": ["新增", "查看", "修改", "删除"],
                    "appFrontIcon": "cuIcon-goods",
                    "buttons": ["查看"],
                    "menu": "旅游攻略列表",
                    "menuJump": "列表",
                    "tableName": "lvyougonglve"
                }],
                "menu": "旅游攻略模块"
            }],
            "hasBackLogin": "否",
            "hasBackRegister": "否",
            "hasFrontLogin": "是",
            "hasFrontRegister": "是",
            "roleName": "用户",
            "tableName": "yonghu"
        }]
    }
}
export default menu;
